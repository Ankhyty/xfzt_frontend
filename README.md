...
- 管理员：拥有作者权限，并可新建季度、修改截止时间、整体更新番剧清单、导出季度内容、修改/删除任意卡片、删除季度
...
- 查看画廊列表
- 按季度筛选卡片与全季跨季度搜索
- 查看卡片详情（支持多图 Lightbox 与长评）
- 个人中心（创作者多季度评测聚合与统计）
- 创建卡片
- 申请上传凭证并直传 OBS
- 提交版本、回滚版本与删除卡片
- 季度列表、季度创建、截止时间设置、番剧清单维护与删除季度
- 季度导出
- 本地缓存与 ETag 校对
...
- 季度名格式为 `YYYY-MM`（月份 01/04/07/10）或支持自定义特辑标识（如 `2026-国漫特辑`）
- 季度番剧清单是前端建卡时的唯一候选来源
